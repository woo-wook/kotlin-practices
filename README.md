...
> 현재는 타입 파라미터의 클래스의 인스턴스 생성, 동반 객체 메서드 호출, 타입 파라미터를 요구하는 함수를 호출하며 실체화 하지 않은 타입 파라미터로 받은 타입을 타입 인자로 넘기기, 클래스, 프로퍼티, 인라인 함수가 아닌 함수의 파라미터를 reified로 지정하기 등이 사용이 불가ㅏ다.
